...
\begin{itemize}[leftmargin=*]
    ...
    However, it is plausible to conjecture the existence of a large deviation principle for these random variables, along with a smooth limiting rate function $G(q)$: \change{indeed, the large deviations rate function (in the weak topology) for the joint law of the spectral measures of $\bW_1$ and $\bW_2$ 
    has been established in~\cite{guionnet2004first}, generalizing Proposition~\ref{prop:ldp_emeasure} from~\cite{arous1997large} for the case of a single Gaussian matrix.}
    ...
    which we have not been able to achieve at present and leave as a direction for future work\footnote{\change{
     A bound $\smallO_d(1)$ for the quantity of eq.~\eqref{eq:required_3rd_moment} can be shown in the case of multi-matrix models with polynomial potentials, as a consequence 
