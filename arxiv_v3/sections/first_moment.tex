...
for completeness, we provide in Appendix~\ref{sec:appendix_ldp} a proof of Corollary~\ref{cor:ldp_Wop_var_principle}
...
\begin{proof}[Proof of Lemma~\ref{lemma:log_pot_mukappa} --]
    ...
    \begin{lemma}\label{lemma:nonc_entropy_pos}
        ...
        Let $\hat{\mu}(t) \coloneqq \int \mu(\rd x) \, e^{i t x}$ be the characteristic function of $\mu$.
        Then 
        \begin{equation*}
