...
\subsubsection{The symmetric binary perceptron}\label{subsubsec:sbp}
...
\subsection{\new{Satisfiability bounds: main results}}
...
\new{
\subsubsection{Sharp second moment asymptotics, and failure of the second moment method}
\todo{
    \begin{itemize}
        \item Non-rigorous asymptotics for the second moment? For it to appear in the main results. 
        I need to move this here.
        \item Change ``Conjecture'' to ``Results''!
    \end{itemize}
}
}
...
In Figure~\ref{fig:phase_diagram}, we plot a sketch of the phase diagram of the problem, as established by Theorem~\ref{thm:first_moment} and Theorem~\ref{thm:second_moment}. 
\begin{figure}[!t]
    \centering
...
Corollary~\ref{cor:sharp_threshold} shows the existence of a sharp threshold $\kappa_c$ depending on $d$, and bounds it in an interval of size $\Theta(1)$. Note that we further expect $\kappa_c(d,\tau)$ to converge to a well-defined limit, as we plan to discuss in a future work.

\myskip
While our results characterize a large part of the $(\kappa, \tau)$ phase diagram, 
we end this section by discussing a few important open directions stemming from our analysis.
...
\myskip
First, the threshold $\tau_2(\kappa)$ is likely not of eq.~\eqref{eq:def_tau2} is optimal, 
...
As shown in Section~\ref{sec:2nd_moment}, sharpening our study of the range of parameters $(\tau, \kappa)$ with bounded second moment ratio is significantly more complex than in the SBP setting:
it requires a precise understanding of the spectral laws of two correlated $\GOE(d)$ matrices, both conditioned on having small spectral norm (see eq.~\eqref{eq:def_Gd}), a very involved problem of large deviations theory applied to random matrices~\citep{guionnet2022rare}.
\fixme{Now this sentence needs to be changed if I want to also do the second moment analysis.}

\myskip
Furthermore, and very interestingly, we \emph{do not expect the second moment method to yield a sharp satisfiability threshold} in average-case matrix discrepancy. 
Concretely, we conjecture
that an equivalent to Theorem~\ref{thm:trans_SBP} can not hold in this problem, in the sense that the satisfiability threshold is not always given by the asymptotics of the first moment of $Z_\kappa$.
\todo{Change this as a Result rather than a theorem?}
\begin{conjecture}[Failure of the second moment method in part of the phase diagram]
    \label{conj:fail_second_moment}
    Let $\kappa \in (0,2]$.
    There exists $\tau_c(\kappa) \geq 0$ such that, 
    for $n, d \to \infty$ with $n/d^2 \to \tau$, if $\tau < \tau_c(\kappa)$, then
    \begin{equation*}
    ...
    Moreover, 
    \begin{equation*}
    \tau_c(\kappa) \geq \frac{1}{2} \left(\frac{\kappa^2}{4} - 1\right)^4,
    \end{equation*}
    and in particular there exists $\kappa \in (0,2)$ such that 
    $\tau_1(\kappa) < \tau_{c}(\kappa)$, so that $\lim (1/d^2) \log \EE[Z_\kappa] > 0$ for $\tau \in (\tau_1(\kappa), \tau_c(\kappa))$.
    }
\end{conjecture}
\noindent
\new{
More precisely, the bound given above implies that $\tau_1(\kappa) < \tau_c(\kappa)$ in the range $0.718 \lesssim \kappa \lesssim 1.652$. 
}
Conjecture~\ref{conj:fail_second_moment} is 
based on a local analysis of the second moment potential $G_d(q)$ of eq.~\eqref{eq:def_Gd} close to $q = 0$ in the high-dimensional limit, which reveals an instability of the 
critical point $q = 0$ for $\tau < \tau_c(\kappa)$.
\new{This non-rigorous analysis further allows to conjecture an exact form for the asymptotics of the second moment in the exponential scale, see Conjecture~\ref{conj:sharp_second_moment} in Section~\ref{sec:limits_2nd_moment}.
}
\todo{
   To stress that the derivation presented in Section~\ref{sec:limits_2nd_moment} is not a rigorous proof, 
   we use the phrasing ``Result'' rather than Theorem, Proposition, or Lemma, which we keep for the results for which we provide a complete mathematical proof.
}
\todo{A remark: this non-rigorous analysis could be established along the same lines, by establishing the large deviations properties of the spectral measures of two correlated GOE matrices. 
While our result is essentially a conjecture of the rate function of this large deviation principle, establishing it rigorously is a technical endeavour that is left for future work.
}
We stress that the statement of Conjecture~\ref{conj:fail_second_moment} is in stark contrast with the symmetric binary perceptron, in 
which the second moment method succeeds in the entirety of the phase diagram (Theorem~\ref{thm:trans_SBP}).
\new{
We sketch the derivation leading to exact second moment asymptotics, and to Conjecture~\ref{conj:fail_second_moment}, in Section~\ref{sec:limits_2nd_moment}. 
Further details will be given in an upcoming companion work, in which we tackle the problem of average-case matrix discrepancy using 
exact but non-rigorous tools of statistical physics.
} 
...
Whether the solution space to average-case matrix discrepancy presents similar geometric features is a stimulating open question, which we leave for future investigation
