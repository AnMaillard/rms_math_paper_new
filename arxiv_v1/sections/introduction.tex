...
Corollary~\ref{cor:sharp_threshold} shows the existence of a sharp threshold $\kappa_c$ depending on $d$, and bounds it in an interval of size $\Theta(1)$. Note that we further expect $\kappa_c(d,\tau)$ to converge to a well-defined limit, as we plan to discuss in a forthcoming publication~\citep{maillard2024average}.
...
\begin{conjecture}[Failure of the second moment method in part of the phase diagram]
    ...
    \end{equation*}
    Moreover, there exists $\kappa \in (0,2)$ such that 
    $\tau_1(\kappa) < \tau_{c}(\kappa)$, so that $\lim (1/d^2) \log \EE[Z_\kappa] > 0$.
\end{conjecture}
\noindent
Conjecture~\ref{conj:fail_second_moment} is 
based on a (non-rigorous) local analysis of the second moment potential $G_d(q)$ of eq.~\eqref{eq:def_Gd} close to $q = 0$ in the high-dimensional limit, which reveals an instability of the 
critical point $q = 0$ for $\tau < \tau_c(\kappa)$.
We carry out this analysis in an upcoming work \cite{maillard2024average}, 
which further reveals that $\tau_c(\kappa) \geq (1/2) (\kappa^2 / 4 - 1)^4$,
so that $\tau_1(\kappa) < \tau_c(\kappa)$ at least in the range $0.718 \lesssim \kappa \lesssim 1.652$. 
...
We will detail the argument leading to Conjecture~\ref{conj:fail_second_moment} in the forthcoming companion work to this manuscript~\citep{maillard2024average}, 
in which we tackle the problem of average-case matrix discrepancy using exact but non-rigorous tools of statistical physics.
